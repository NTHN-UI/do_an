                        ...
                        <div class="row g-4 align-items-end">
                            <div class="col-md-5">
                                ...
                                <span class="input-group-text bg-white border-primary-subtle rounded-start-3">
                                    <i class="fas fa-search text-primary-color opacity-75"></i> {{-- Changed text-primary to text-primary-color --}}
                                </span>
                                ...
                                <div class="d-flex flex-wrap gap-2 align-items-center">
                                    ...
                                    @foreach($subjects->take(3) as $subject)
                                        <a href="{{ route('documents.index', ['subject_id' => $subject->id]) }}"
                                           class="badge bg-primary-subtle text-primary-color text-decoration-none px-3 py-2 rounded-pill hover-shadow"> {{-- Changed text-primary to text-primary-color --}}
                                            {{ $subject->name }}
                                        </a>
                                    @endforeach
                                </div>
                ...
                <div class="alert alert-info border-0 rounded-3 bg-info-subtle shadow-sm">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            @if(request('search'))
                                cho từ khóa "<strong class="text-primary-color">{{ request('search') }}</strong>" {{-- Changed text-primary to text-primary-color --}}
                            @endif
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-6 col-lg-4">
                        <div class="document-card card h-100 border-0 shadow-sm rounded-4 overflow-hidden hover-lift-card">
                            <div class="card-header bg-white border-0 pb-0 position-relative">
                                ...
                                <span class="badge bg-primary-color rounded-pill px-3 py-2 fw-semibold"> {{-- Changed bg-primary to bg-primary-color --}}
                            ...
                            <div class="card-body d-flex flex-column p-4">
                                <h5 class="card-title fw-bold mb-3 text-primary-color lh-base">{{ $document->title }}</h5> {{-- Changed text-primary to text-primary-color --}}
                                ...
                                <div class="teacher-info d-flex align-items-center p-3 bg-light rounded-3 mb-3">
                                    <div class="teacher-avatar bg-primary bg-opacity-10 text-primary-color rounded-circle d-flex align-items-center justify-content-center me-3" {{-- Changed bg-primary/text-primary to bg-primary-color/text-primary-color --}}
                                ...
                                <div class="d-flex align-items-center justify-content-between">
                                    @if(Auth::user()->isStudent())
                                        <a href="{{ route('documents.download', $document) }}"
                                           class="btn btn-primary-color rounded-pill px-4 py-2 fw-semibold hover-lift"> {{-- Changed btn-primary to btn-primary-color --}}
                    ...
                    <div class="col-12">
                        <div class="empty-state text-center py-5">
                            ...
                            <div class="d-flex gap-3 justify-content-center">
                                <a href="{{ route('documents.index') }}" class="btn btn-primary-color rounded-pill px-4 hover-lift"> {{-- Changed btn-primary to btn-primary-color --}}
                                    <i class="fas fa-refresh me-2"></i>Xóa bộ lọc
                                </a>
                                @if(Auth::user()->isTeacher())
                                    <a href="{{ route('documents.create') }}" class="btn btn-outline-primary rounded-pill px-4 hover-lift">
